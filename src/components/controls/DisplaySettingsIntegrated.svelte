...
function toggleSakuraEnabled() {
	...
	window.dispatchEvent(
		new CustomEvent("sakuraToggleChange", {
			detail: { enabled: sakuraEnabled },
		}),
	);
}

function resetSakuraEnabled() {
	sakuraEnabled = defaultSakuraEnabled;
	setSakuraEnabled(defaultSakuraEnabled);
	window.dispatchEvent(
		new CustomEvent("sakuraToggleChange", {
			detail: { enabled: defaultSakuraEnabled },
		}),
	);
}
    ...
    {#if isSakuraSwitchable}
        <div class="mt-2 mb-2">
            <div class="flex gap-2 font-bold text-lg text-neutral-900 dark:text-neutral-100 transition relative ml-3 mb-2
                ...
                before:absolute before:-left-3 before:top-1/2 before:-translate-y-1/2"
            >
                {i18n(I18nKey.sakuraAnimation)}
                <button aria-label="Reset to Default" class="btn-regular w-7 h-7 rounded-md active:scale-90"
                        class:opacity-0={sakuraEnabled === defaultSakuraEnabled} class:pointer-events-none={sakuraEnabled === defaultSakuraEnabled} onclick={resetSakuraEnabled}>
                    <div class="text-(--btn-content)">
                        <Icon icon="fa7-solid:arrow-rotate-left" class="text-[0.875rem]"></Icon>
                    </div>
                </button>
            ...
            <div class="space-y-1">
                ...
                >
                    <Icon icon="mdi:flower" class="text-[1.25rem] shrink-0"></Icon>
                    <span class="text-sm flex-1">{i18n(I18nKey.sakuraAnimation)}</span>
                    <div class="w-10 h-5 rounded-full transition-all duration-200 relative"
